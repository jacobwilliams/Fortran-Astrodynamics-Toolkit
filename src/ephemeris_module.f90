...
!  Abstract class for celestial body ephemerides.
    ...
    contains
        ...
        procedure(r_func),deferred,public :: get_r  !! get the position of one body w.r.t. another body.
    ...
    abstract interface
        ...
        subroutine r_func(me,et,targ,obs,r,status_ok)
        ...
        type(celestial_body),intent(in)      :: targ       !! target body
        type(celestial_body),intent(in)      :: obs        !! observer body
        real(wp),dimension(3),intent(out)    :: r          !! position vector of targ w.r.t. obs
        logical,intent(out)                  :: status_ok  !! true if there were no problems
        end subroutine r_func
    end interface
