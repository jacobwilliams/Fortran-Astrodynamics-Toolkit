...
!   * [[gravity_test]]
    ...
    real(wp),dimension(3),parameter :: v_chaser_lvlh = [zero,zero,zero]       !! chaser v relative to target (LVLH)
    ...
    real(wp)                :: t0,tf,dt 
    logical                 :: status_ok
    integer                 :: iunit,istat,iunit_cw
    logical                 :: cw_header_written
    
    cw_header_written = .false.
        
    !open output file:
    open(newunit=iunit,file   ='cw_test_integration.txt',status='REPLACE',iostat=istat)
    ...
    write(*,*) ''
    write(*,'(A/,*(E30.16/))') 'rv_target_eci_tf (integrated):',rv_target_eci_tf
    ...
    call plt%initialize(grid=.true.,xlabel='LVLH x [km]',ylabel='LVLH y [km]',&
                        title='Integration vs CW Propagation',legend=.true.)
    call plt%add_plot(x_lvlh_int,y_lvlh_int,label='Integrated',linestyle='b-',markersize=5,linewidth=2)
    call plt%add_plot(x_lvlh_cw,y_lvlh_cw,label='CW Equations',linestyle='r-',markersize=5,linewidth=2)
    call plt%savefig('cw_test_xy.png')
    call plt%destroy()

    call plt%initialize(grid=.true.,xlabel='LVLH x [km]',ylabel='LVLH z [km]',&
                        title='Integration vs CW Propagation',legend=.true.)
    call plt%add_plot(x_lvlh_int,z_lvlh_int,label='Integrated',linestyle='b-',markersize=5,linewidth=2)
    call plt%add_plot(x_lvlh_cw,z_lvlh_cw,label='CW Equations',linestyle='r-',markersize=5,linewidth=2)
...
!*****************************************************************************************
    ...
    !*********************************************************
        ...
        !!
        !! The gravity frame is assumed to be the IAU_EARTH frame.
    ...
    !*********************************************************
        ...
        class is (spacecraft)
            ...
            !output states:
            write(iunit,'(*(F25.6,1X))') t,x,rv_chaser_lvlh
